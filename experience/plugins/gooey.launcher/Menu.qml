// Derived from Omarchy shell/plugins/menu/Menu.qml. See LICENSE and NOTICE.md.
import Quickshell
...
import "AnchorResolver.js" as AnchorResolver
...
Item {
  ...
  readonly property var anchorService: service || (shell ? shell.serviceFor("gooey.launcher") : null)
  onAnchorServiceChanged: if (anchorService) anchorService.panel = root
  Component.onCompleted: if (anchorService) anchorService.panel = root

  // One keepLoaded controller owns menu state for every output's button.
  property string pendingInitialMenu: "root"
  property Item selectedWidget: null
  readonly property Item anchorItem: selectedWidget ? selectedWidget.anchorItem : null
  readonly property var anchorBar: selectedWidget ? selectedWidget.bar : null
  property string anchorToken: ""
  property string anchorOutput: ""
  property var popoutBar: null
  property double missingAnchorSince: 0
  property string lastFallback: ""
  readonly property bool anchorReady: root.outputPresent(root.anchorOutput) && root.widgetUsable(root.selectedWidget)

  function widgetUsable(widget) {
    if (!widget || !widget.bar || widget.bar.barHidden || !widget.anchorItem) return false
    var anchor = widget.anchorItem
    var win = anchor.QsWindow.window
    if (!win || !win.screen || !win.visible || !win.backingWindowVisible) return false
    if (!widget.visible || widget.opacity <= 0 || widget.width <= 0 || widget.height <= 0) return false
    if (!anchor.visible || anchor.opacity <= 0 || anchor.width <= 0 || anchor.height <= 0) return false
    // Center-aligned modules have a zero-sized placeholder in another slot.
    for (var ancestor = anchor.parent; ancestor && ancestor !== win.contentItem; ancestor = ancestor.parent) {
      if (!ancestor.visible || ancestor.opacity <= 0 || ancestor.width <= 0 || ancestor.height <= 0) return false
    }
    return true
  }
  ...
  function candidates() {
    ...
    var widgets = root.anchorService ? root.anchorService.widgets : []
    var result = []
    for (var i = 0; i < widgets.length; i++) {
      var widget = widgets[i]
      if (!root.widgetUsable(widget)) continue
      var item = widget.anchorItem
      var win = item.QsWindow.window
      var p = item.mapToItem(win.contentItem, 0, 0)
      result.push({ widget: widget, token: widget.anchorToken,
        output: String(win.screen.name || ""), position: widget.bar.vertical ? p.y : p.x })
    }
    return result
  }
  ...
  function setAnchor(candidate) {
    ...
    var nextBar = candidate.widget.bar
    if (root.popoutBar && root.popoutBar !== nextBar && root.popoutBar.releasePopout)
      root.popoutBar.releasePopout(root)
    root.popoutBar = nextBar
    root.selectedWidget = candidate.widget
    root.anchorToken = candidate.token
    root.anchorOutput = candidate.output
    root.missingAnchorSince = 0
    if (root.opened && nextBar && nextBar.requestPopout) nextBar.requestPopout(root)
    return true
  }

  function outputPresent(name) {
    var screens = Quickshell.screens
    for (var i = 0; i < screens.length; i++) {
      if (screens[i].name === name) return true
    }
    return false
  }
  ...
  function checkAnchor() {
    ...
    if (!root.outputPresent(root.anchorOutput)) { root.cancel(); return }
    if (root.anchorReady) { root.missingAnchorSince = 0; return }
    // Suppress the input surface immediately while a bar surface is rebuilt.
    // Reacquire on the same output for up to 600ms, keeping navigation/search.
    var next = AnchorResolver.choose(root.candidates(), "", root.anchorOutput)
    if (next && next.output === root.anchorOutput) { root.setAnchor(next); return }
    if (!root.missingAnchorSince) root.missingAnchorSince = Date.now()
    if (Date.now() - root.missingAnchorSince >= 600) root.cancel()
  }
  ...
  function open(payloadJson) {
    ...
    if (payload.mode === "select" || payload.mode === "input") {
      // Modal utility requests continue using the stock implementation.
      root.cancel()
      if (root.shell) root.shell.summon("omarchy.menu", payloadJson)
      return "stock-utility"
    }

    var pointer = payload.trigger === "pointer"
    if (payload.toggle === true && root.opened && (!pointer || payload.anchorToken === root.anchorToken)) {
      root.cancel()
      return "closed"
    }
    var hostBar = root.shell ? root.shell.bar : null
    var focusedOutput = Hyprland.focusedMonitor ? Hyprland.focusedMonitor.name : ""
    var next = AnchorResolver.choose(root.candidates(), pointer ? String(payload.anchorToken || "") : "",
      pointer ? String(payload.output || "") : focusedOutput)
    if (!next) {
      root.cancel()
      root.lastFallback = "No visible Gooey launcher button; opening the stock Omarchy menu."
      console.warn("gooey.launcher:", root.lastFallback)
      if (root.shell) root.shell.summon("omarchy.menu", JSON.stringify({ menu: payload.initialMenu || payload.menu || "root" }))
      return "stock-fallback"
    }

    var preserve = root.opened && pointer
    root.setAnchor(next)
    root.lastFallback = ""
    if (preserve) {
      Qt.callLater(function() { if (root.opened) searchInput.forceActiveFocus() })
      return "reanchored"
    }
  ...
  function toggle(route) {
    return root.open(JSON.stringify({ trigger: "keyboard", toggle: true, menu: route || "root" }))
  }

  function status() {
    return JSON.stringify({ opened: root.opened, visible: panel.open, output: root.anchorOutput,
      anchorToken: root.anchorToken, anchorReady: root.anchorReady,
      barPosition: panel.barPos, card: { x: panel.cardOrigin.x, y: panel.cardOrigin.y,
        width: panel.contentWidth, height: panel.contentHeight },
      bar: { width: panel.barW, height: panel.barH },
      anchor: { x: panel.anchorScreenPos.x + (panel.barPos === "right" ? panel.screenW - panel.barW : 0),
        y: panel.anchorScreenPos.y + (panel.barPos === "bottom" ? panel.screenH - panel.barH : 0),
        width: panel.anchorW, height: panel.anchorH },
      screen: { width: panel.screenW, height: panel.screenH },
      menu: root.activeMenu, filter: root.filterText, rows: displayModel.count,
      fallback: root.lastFallback,
      searchFocused: searchInput.activeFocus,
      deleteConfirmation: root.deleteConfirmOpen,
      controls: { search: root.controlRect(searchInput), back: root.controlRect(backButton),
        close: root.controlRect(closeButton), clear: root.controlRect(clearButton) },
      items: root.visibleItemRects() })
  }

  function controlRect(item) {
    if (!item || !item.visible) return null
    var p = item.mapToItem(card, 0, 0)
    return { x: p.x + panel.cardOrigin.x, y: p.y + panel.cardOrigin.y, width: item.width, height: item.height }
  }
  function visibleItemRects() {
    var out = [], children = resultList.contentItem.children
    for (var i = 0; i < children.length; i++) {
      var item = children[i]
      if (item.objectName !== "gooey-launcher-row") continue
      var p = item.mapToItem(resultList, 0, 0)
      if (p.y >= 0 && p.y + item.height <= resultList.height + 1)
        out.push({ label: item.label, itemId: item.itemId, rect: root.controlRect(item) })
    }
    return out
  }

  IpcHandler {
    target: "gooey.launcher"
    function toggle(route: string): string { return root.toggle(route) }
    function summon(route: string): string { return root.open(JSON.stringify({ trigger: "keyboard", menu: route || "root" })) }
  ...
  Timer {
    ...
    repeat: true
    running: root.opened
    onTriggered: root.checkAnchor()
  }

  Connections {
    target: root.selectedWidget
    ignoreUnknownSignals: true
    function onAnchorItemChanged() { Qt.callLater(root.checkAnchor) }
    function onAnchorWindowChanged() { Qt.callLater(root.checkAnchor) }
  }
  ...
  onOpenedChanged: if (!opened) {
    ...
    deleteTarget = null
    // A removed anchor can clear KeyboardPanel.bar before its close handler.
    // Retain and release the previous coordinator explicitly in that case.
    if (root.popoutBar && root.popoutBar.releasePopout) root.popoutBar.releasePopout(root)
    root.popoutBar = null
  }
  Component.onDestruction: {
    if (anchorService && anchorService.panel === root) anchorService.panel = null
    if (root.popoutBar && root.popoutBar.releasePopout) root.popoutBar.releasePopout(root)
  }
  ...
  property int headerHeight: Style.space(96)
  ...
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : ((root.activeMenu === "trigger.capture.screenrecord" || root.activeMenu === "style.font") ? Style.space(520) : Style.space(360)), Math.max(1, panel.availableCardWidth))
  property int visibleRowsHeight: root.dmenuActive ? dmenuRowListHeight(layoutSerial, displayModel.count, filterText) : rowListHeight(layoutSerial, displayModel.count, filterText, searchDivider)
  // On side bars, a stable viewport prevents search from moving the card
  // vertically around its button. Top/bottom cards grow away from the bar.
  readonly property bool sideBar: panel.barPos === "left" || panel.barPos === "right"
  readonly property int sideViewportHeight: Math.max(1, Math.min(Style.space(560), panel.availableCardHeight))
  readonly property int cardInsets: root.contentMargin * 2 + Border.top(root.borderSpec) + Border.bottom(root.borderSpec)
  property int cardHeight: root.sideBar
    ? root.sideViewportHeight
    : Math.max(1, Math.min(root.cardInsets + headerHeight + contentSpacing + visibleRowsHeight, panel.availableCardHeight))
  ...
  // Available space is measured inward from the live bar, never from a
  // frozen screen-center coordinate. Row overflow stays mouse-scrollable.
  function availableRowsHeight() {
    var height = root.sideBar ? root.sideViewportHeight : panel.availableCardHeight
    return Math.max(0, Math.min(height - root.cardInsets - root.headerHeight - root.contentSpacing,
      Math.round(panel.screenH * 0.7)))
  }
  ...
  function rowListHeight(_serial, _count, _filter, _divider) {
    if (displayModel.count === 0) return Math.min(root.baseRowHeight, availableRowsHeight())
    ...
    return Math.max(0, Math.min(foldedListHeight(totals, availableRowsHeight()), availableRowsHeight()))
  }
  ...
  KeyboardPanel {
    ...
    bar: root.anchorBar
    owner: root
    open: root.opened && root.rowsLoaded && root.anchorReady
    // Invalidated anchors must not leave even a fading input surface behind.
    visible: open
    centerOnBar: false
    ...
    WlrLayershell.namespace: "gooey-launcher"
      ...
      Column {
        ...
        Rectangle {
          ...
          PanelActionButton {
            ...
            onClicked: { root.goBack(); searchInput.forceActiveFocus() }
          }
          ...
          Text {
            ...
            anchors.left: parent.left
            anchors.leftMargin: backButton.visible ? backButton.width + Style.spacing.xs : 0
            anchors.right: parent.right
            anchors.rightMargin: closeButton.width + Style.spacing.xs
            height: Style.space(44)
            verticalAlignment: Text.AlignVCenter
            text: root.activeMenu === "root" ? "Apps & desktop" : (root.item(root.activeMenu) ? (root.item(root.activeMenu).title || root.item(root.activeMenu).label) : "Menu")
          ...
          TextField {
            ...
            placeholderText: root.dmenuActive ? root.dmenuPrompt : "Search apps and actions"
            font.family: root.fontFamily
            font.pixelSize: Style.font.body
            foreground: root.foreground
            rightPadding: (clearButton.visible ? clearButton.width + Style.spacing.sm : horizontalPadding)
              + Border.right(Border.controlSpec(activeFocus ? "focus" : (hovered || hasCursor ? "hover-cursor" : "normal"), foreground, accent))
            selectByMouse: true
            Accessible.name: "Search apps and actions"
            ...
            Keys.onPressed: function(event) {
              if (event.key === Qt.Key_PageUp || event.key === Qt.Key_PageDown) {
                root.select(event.key === Qt.Key_PageUp ? -6 : 6)
                event.accepted = true
              }
            }
            onAccepted: {
            ...
            PanelActionButton {
              ...
              onClicked: { root.setFilter(""); searchInput.forceActiveFocus() }
            }
          }
        ...
        Item {
          width: parent.width
          height: Math.max(0, Math.min(root.visibleRowsHeight, card.height - root.cardInsets - root.headerHeight - root.contentSpacing))
